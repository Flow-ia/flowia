...
export function AuthPanel({ slug, th, onAuth, onClose, requireAccount, initialEmail = '', initialMode = 'login', referralCode = '', quickMode = false, onModeChange }) {
  ...
  const S = {
    ...
    btnPrimary: { width:'100%', padding:'13px', borderRadius:99, border:'none',
                  background:th.accent, color:th.accentText, fontWeight: 500,
                  fontSize:14, cursor:'pointer', letterSpacing:'-0.01em' },
    btnSecondary: { width:'100%', padding:'13px', borderRadius:99,
      ...
      <div style={{ padding:20 }}>
        ...
        {(mode === 'login' || mode === 'register') && (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            ...
            {/* Bouton Google PRIMAIRE — mis en avant. Commit 22 : compte
            ...
            <p style={{ margin:'-4px 0 4px', fontSize:11, color:'#6b7280',
              textAlign:'center', letterSpacing:'-0.005em' }}>
              ✓ En 5 secondes · Vos données sécurisées · 1 clic pour les RDV futurs
            </p>

            {/* Séparateur "ou créer un compte par email" */}
            <div style={{display:'flex',alignItems:'center',gap:10,margin:'6px 0 2px'}}>
              <div style={{flex:1,height:1,background:th.border}}/>
              <span style={{fontSize:11,color:'#9ca3af',whiteSpace:'nowrap',padding:'0 6px'}}>
                ou {mode==='login' ? 'se connecter par email' : 'créer un compte par email'}
              </span>
              <div style={{flex:1,height:1,background:th.border}}/>
            </div>
            ...
            {/* Bouton submit email/password — RGPD commit 20 : phone valide obligatoire en register */}
